...
char port_read_char(sexp port) {
  /* FILE *stream = in_port_stream(port); */
  /* return fgetc(stream); */
  return read_C_char(port);
}

void unget_C_char(char c, sexp port) {
  assert(is_in_port(port));
  FILE *stream = in_port_stream(port);
  ungetc(c, stream);
}
...
sexp read_float(int integer, sexp port) {
  int number = 0;
  int digit, i = 1;
  while (isdigit(digit = /* port_read_char */read_C_char(port))) {
    number = number * 10 + digit - '0';
    i = i * 10;
  }
  /* port_ungetc(digit, port); */
  unget_C_char(digit, port);
...
sexp read_number(char c, int sign, sexp port) {
  int number = c - '0';
  int digit;
  while (isdigit(digit = read_C_char(port))) {
    number = number * 10 + digit - '0';
  }
  if (digit == '.')
...
void read_comment(sexp port) {
  int c = /* port_read_char */read_C_char(port);
  while (c != '\n' && c != EOF)
    c = /* port_read_char */read_C_char(port);
}
...
tail_loop:
  ...
  switch (c) {
    ...
    case ';': read_comment(port); /* return read_object(port); */goto tail_loop;
